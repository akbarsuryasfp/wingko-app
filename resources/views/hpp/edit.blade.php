    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <h4 class="mb-3 text-primary">
                <i class="bi bi-calculator"></i> Edit HPP: {{ $detail->produk->nama_produk }} ({{ $detail->no_detail_produksi }})
            </h4>
            @if ($errors->any())
            ...
            <form action="{{ route('hpp.update', $detail->no_detail_produksi) }}" method="POST" id="hppForm">
                @csrf
                @method('PUT')
                                ...
                                <tbody>
                                    @foreach ($resep as $i => $r)
                                        @php
                                            ...
                                            $hpp = $bahanHpp[$r->kode_bahan] ?? null;
                                        @endphp
                                        <tr>
                                            <td>
                                                {{ $r->bahan->nama_bahan }}
                                            ...
                                            <td>
                                                <input type="number" class="form-control" name="bahan[{{ $i }}][jumlah]" value="{{ $hpp->jumlah_bahan ?? $jumlah }}" step="0.01" min="0">
                                            </td>
                                            <td>
                                                <input type="number" class="form-control" name="bahan[{{ $i }}][harga]" step="0.01" min="0" value="{{ $hpp->harga_bahan ?? $r->bahan->harga_satuan }}">
                                            </td>
                                            <td>
                                                <input type="number" class="form-control" name="bahan[{{ $i }}][total]" value="{{ $hpp->total_bahan ?? '' }}" readonly>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            ...
                            <table class="table table-bordered align-middle">
                                ...
                                <tbody>
                                    @foreach ($karyawan as $i => $k)
                                        @php $tk = $tkHpp[$k->kode_karyawan] ?? null; @endphp
                                        <tr>
                                            <td>
                                                {{ $k->nama }}
                                            ...
                                            <td>
                                                <input type="number" class="form-control" name="tk[{{ $i }}][jam]" step="0.1" min="0" value="{{ old('tk.'.$i.'.jam', $tk->jumlah_jam ?? 0) }}">
                                            </td>
                                            <td>
                                                <input type="number" class="form-control" name="tk[{{ $i }}][tarif]" step="0.01" min="0" value="{{ $tk->tarif_per_jam ?? $k->tarif_per_jam }}">
                                            </td>
                                            <td>
                                                <input type="number" class="form-control" name="tk[{{ $i }}][total]" value="{{ $tk->total_btkl ?? '' }}" readonly>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                    ...
                    <div class="tab-pane fade" id="overhead" role="tabpanel">
                        <div class="table-responsive">
                            <table class="table table-bordered align-middle">
                                ...
                                <tbody>
                                    @foreach ($bop as $i => $o)
                                        @php $ov = $bopHpp[$o->kode_bop] ?? null; @endphp
                                        <tr>
                                            <td>
                                                {{ $o->nama_bop }}
                                            ...
                                            <td>
                                                <input type="number" class="form-control" name="overhead[{{ $i }}][biaya]" step="0.01" min="0" value="{{ $ov->biaya_bop ?? '' }}">
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                ...
                <div class="d-grid mt-4">
                    <button type="submit" class="btn btn-success btn-lg shadow-sm">
                        <i class="bi bi-save"></i> Update HPP
                    </button>
                </div>
            </form>
        </div>
